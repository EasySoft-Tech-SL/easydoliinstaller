...
if (isset($_GET['ajax'])) {
    ...
    if ($ajax === 'reparar') {
        ...
        // which = mod (solo modificados) | miss (solo ausentes) | all (ambos).
        $which = $_GET['which'] ?? ($_POST['which'] ?? 'all');
        $mod = $res['modified'] ?? array();
        $miss = $res['missing'] ?? array();
        if ($which === 'mod') {
            $files = $mod;
        } elseif ($which === 'miss') {
            $files = $miss;
        } else {
            $files = array_merge($mod, $miss);
        }
...
function di_package_picker($prev, $zips, $minVer = '')
{
    ...
    // Versión SELECCIONADA en el desplegable (envío anterior o preselección de la config).
    $prevSelect = isset($_POST['download_version']) ? (string) $_POST['download_version'] : (string) ($prev['download_version'] ?? '');
    // Versión MANUAL: solo lo que el usuario escribió a mano (vacío por defecto). Es un
    // override; el handler le da prioridad si no está vacío, así que NO se pre-rellena
    // (si no, cambiar el desplegable no tendría efecto).
...
<div class="win"><div class="t"><?php echo di_h(di_t('pk_title')); ?></div><div class="b">
    ...
    <div id="blk_dl" style="margin-top:12px">
        ...
        <select name="download_version" id="dlver">
            ...
            // Inyecta la versión preseleccionada (p. ej. la instalada en reparar) si no
            // está en la lista, para que sea seleccionable.
...
<?php } else {
    ...
    <div class="row" style="margin-top:8px"><span></span><button class="btn amber" id="dorepmod" onclick="reparMod()"><?php echo di_h(di_t('ir_repmod', array('{n}' => count($modified)))); ?></button></div>
    <?php } ?>
    <?php if ($missing) { ?>
    ...
    ?></pre>
    <div class="row" style="margin-top:8px"><span></span><button class="btn amber" id="dorepmiss" onclick="reparMiss()"><?php echo di_h(di_t('ir_repmiss', array('{n}' => count($missing)))); ?></button></div>
    <?php } ?>
    <?php if ($extra) { ?>
...
<script>
  var T=<?php echo json_encode(array(
        'confirmmod' => di_t('ir_confirmmod', array('{n}' => count($modified))),
        'confirmmiss' => di_t('ir_confirmmiss', array('{n}' => count($missing))),
