...
Detects an installed Dolibarr and offers **Repair / verify integrity**. Pick the **official package of the same version** (download or local ZIP), then choose **Verify files** or **Repair database** — two buttons right on the Repair page.

**Verify files** then:

1. Compares the install **file-by-file** against the official package (by hash), in chunks with a live log.
2. Shows a **detailed report by sections** — **modified** (`~`), **missing** (`+`) and **unexpected/injected** (`!`, present locally but not in the official package, e.g. a planted webshell). Only your data and config are excluded (`conf/`, `custom/`, `documents/`, plus the installer's own `install/install.forced.php` and lock files). The `install/` directory **is** checked — Dolibarr keeps it after install (just locked by `install.lock`), so a missing or altered `install/` is reported and can be restored. Each **modified file expands to a line-by-line diff** (red = official, green = your install).
...
**Repair database** — recreate missing tables and reference data by running Dolibarr's native step (`CREATE TABLE IF NOT EXISTS`), the automated equivalent of Dolibarr's *"First install"* repair. It never creates the DB/user, never touches the admin and never rewrites `conf.php` (confirmation + optional DB backup first). With a local package it jumps straight there; you don't have to run the file check first.
